...
async function _supaUpdateUser(payload) {
  ...
  if (!fullName) return {
    ok: false,
    error: "A user needs a full name."
  };
  ...
  if (!av) return {
    ok: false,
    error: "Refusing to save an empty permission set — a user with no grant falls back to their role, which for an admin is every menu. Pick at least one menu, or choose Full access."
  };
  const row = {
    full_name: fullName,
    position: String(payload && payload.position || ""),
  ...
  const {
    data: hit,
    error
  } = await sb.from("erp_users").update(row).eq("id", id).select("id");
  if (error) return {
    ok: false,
    error: "Could not save this user: " + error.message
  };
  if (!hit || !hit.length) return {
    ok: false,
    error: "Saving this user was refused by the database — your account may not have permission. Nothing changed."
  };
  return {
    ok: true,
    id,
    password_ignored: !!String(payload && payload.password || "").trim()
  };
}
...
function UserModal({
  ...
  const save = async () => {
    ...
    if (res && res.ok) {
      ...
      onSaved(res.password_ignored ? `Saved — but the password was NOT changed. Use the Reset password button on ${form.username}'s row.` : "");
      return;
    }
    ...
    try {
      console.error("update_user failed:", {
        id: user && user.id,
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Weekly payroll membership, daily rate and work schedule are managed in ", /*#__PURE__*/React.createElement("b", {
    style: {
      color: t.textMuted
    }
  }, "Payroll \u2192 Roster"), ". This screen does not load a user's real pay rate, so it cannot safely edit one.")), /*#__PURE__*/React.createElement("div", {
    style: {
      gridColumn: "1 / -1",
      marginTop: 2,
      paddingTop: 10,
    ...
    style: {
      ...
      padding: "9px 11px",
      borderRadius: 10,
      background: (t.bad || "#b4232a") + "1A",
      border: `1px solid ${t.bad || "#b4232a"}`,
      color: t.bad || "#b4232a",
      fontSize: 12,
      fontWeight: 600,
      lineHeight: 1.45
    }
  }, "Not saved \u2014 ", err), /*#__PURE__*/React.createElement("div", {
    className: "flex justify-end gap-2 px-5 py-4",
    style: {
      borderTop: `1px solid ${t.border}`
    }
...
function SettingsPage({
  ...
  const onSaved = msg => {
    setModal(null);
    setFlash(msg || "Saved");
    setTimeout(() => setFlash(""), msg ? 8000 : 2500);
    refresh();
  };
